/* 
    ...
    ON 테이블1.공통칼럼 = 테이블2.공통칼럼;
    
    WHERE 구문을 이용해 ON절로 합쳐진 결과 칼럼에 대한 필터링이 가능하다.
*/
...
INSERT INTO purchase_tbl VALUES
    ...
    (null, 5, '2023-05-14', 40000),
    (null, 5, '2023-05-15', 60000),
    (null, 1, '2023-05-16', 15000);
...
-- 원래 있던 buy_tbl과 user_tbl을 조인해주세요.
-- user_tbl에서 조회할 칼럼 : user_name, user_address, user_num
-- buy_tbl에서 조회할 칼럼 : buy_num, prod_name, prod_cate, price, amount
SELECT user_tbl.user_name, user_tbl.user_address, user_tbl.user_num,
	   buy_tbl.but_num, buy_tbl.prod_name, buy_tbl.prod_cate, buy_tbl.price, buy_tbl.amount
FROM user_tbl INNER JOIN buy_tbl
ON user_tbl.user_num = buy_tbl.user_num;

-- 테이블명을 하나하나 작성하면 귀찮기 때문에, 테이블명을 별칭으로 줄여서 작성해보겠습니다.
-- FROM절에서 테이블명을 지정할 때,
-- FROM 테이블명 별칭1 JOIN 테이블명 별칭2 형식을 사용하면 
-- 테이블명을 대신하여 사용할 수 있습니다.
...
-- 따라서 뒤에서 배울 UNION이라는 구무을 응용해 처리한다.alter
...
-- CROSS JOIN은 조인 대상인 테이블1과 테이블2 간의 모든 ROW의 조합쌍을 출력합니다.
-- 그래서 결과 ROW의 개수는 테이블1의 ROW개수 * 테이블2의 ROW개수가 됩니다.

-- 테스트 코드를 돌려보고, 예제 테이블을 다시 만들어보겠습니다.
SELECT u.user_num, b.buy_num FROM user_tbl u, buy_tbl b; -- ,로 나열 시 cross join 구문 생략 가능 
              
-- user_tbl 10개 로우, buy_tbl은 3개 로우 = 크로스 조인 결과 로우는 총 30개
SELECT COUNT(*) FROM user_tbl CROSS JOIN buy_tbl;
...
-- 크로스조인 결과를 보면서 모든 조합쌍이 나왔는지 확인해보세요.
SELECT * FROM phone_volume CROSS JOIN phone_color;

-- SELF 조인은 자기 테이블 내부 자료를 참조하는 컬럼이 있을 때
-- 해당 자료를 온전하게 노출시키기 위해서 사용하는 경우가 대부분입니다.
-- 예시로는 사원 목록 중 자기 자신과 직속 상사를 나타내거나 게시판에서 원본글과 답변을 나타내는 경우에 
...
-- SELF JOIN을 이용해 직원이름과 상사이믈이 같이 나오게 만들어보겠습니다.
...
-- UNION은 SELECT 문 두 개를 위 아래로 배치해서 양쪽 결과를 붙여줍니다.
-- 수직적 확장을 고려할 때 주로 사용합니다.
-- 고양이 테이블 생성
CREATE TABLE CAT (
