/* MySQL에서도 프로그래밍이 가능함.
JAVA, PYTHON 등과 달리 제약이 많지만 반복 작업을 줄여줄 수 있다는 점에서 많이 사용한다. */

/*	<MySQL에서 변수 사용하기>
	변수를 지정할 때에는 SET @변수명 = 값; 의 문법을 사용합니다. */
SET @myVar1 = 5;
SET @myVar2 = 3;
SET @myVar3 = 4.25;
SET @myVar4 = '유저명 : ';
SET myVar5 = 50; -- 변수명 왼쪽에 @을 작성 안하면......?
...
-- 프로시저 삭제 시 DROP PROCEDURE 프로시저명; -- ()는 쓰지 않는다!!!!
-- 특정 프로시저는 DB에 소속된 것이기 때문에 반드시 DB가 먼저 선택되어야 명령을 수행합니다.
...
BEGIN
    ...
    SELECT entry_date INTO entryDate #쿼리문의 결과로 나온 값을 entryDate 변수에 대입
		FROM user_tbl WHERE user_num = 22;
        
	SET todayDate = CURRENT_DATE(); # 오늘 날짜를 얻어오는 내장함수, 오늘 날짜만 얻어옴,
    SET days = DATEDIFF(todayDate, entryDate); # 경과 일수 구하기
    
    IF(days/365) >= 3 THEN
		SELECT CONCAT('가입한지', days, '일이 경과했습니다.');
	ELSE
		SELECT CONCAT('가입한지 3년 미만이며 ', days, '일이 경과했습니다,');
	END IF;
END $$
DELIMITER ;
CALL getThree();

-- user_num 다시 지정하려면 DROP PROCEDURE 하고 새로운 번호로 다시 저장해야 함.
...
BEGIN
	...
	IF point >= 90 THEN SET ranking = 'A';
	ELSEIF point >= 80 THEN SET ranking = 'B';
	ELSEIF point >= 70 THEN SET ranking = 'C';
	ELSEIF point >= 60 THEN SET ranking = 'D';
	ELSE SET ranking = 'F';
	END IF;

	SELECT CONCAT('취득점수: ', point, ', 학점: ', ranking) AS result;
END $$
DELIMITER ;
...
/* 프로시저 getThree()의 문제점은 조회하려는 히원번호를 바꿀 때마다
프로시저를 삭제했다가 번호를 갱신한 프로시저를 다시 저장해주어야 합니다,
...
-- 게임을 삭제할 때마다 게임이 삭제되었다는 메세지를 추가하기
DELIMITER //
CREATE TRIGGER testTrg
	AFTER DELETE 	# 삭제된 후에
    ON game_list	# game_list 테이블으 ㅣ자료가
    FOR EACH ROW
BEGIN
	# OLD.컬럼명을 적으면 해당 삭제 row의 데이터를 조죄할 수 있습니다.
	SET @msg = CONCAT(OLD.game_name, '게임이 시작되었습니다.');
END //
DELIMITER ;
