-- SUB QUERYFKS 쿼리문 내에 쿼리문을 중첩하는 것입니다. 
-- 기본적으로 SELECT문의 범위를 좁히는 경우에 많이 사용합니다.

-- 회원별 평균 키를 구하는 GROUP BY 구문을 수행하겠습니다.
...
GROUP BY user_name;
-- 위에서 설명했듯, GROUP BY는 우선 동명이인을 하나의 집단으로 보기 때문에 문제가 되고,
-- 하나의 테이블에 전체 평균을 넣을 수가 없습니다.
-- 이 경우 서브쿼리를 이용하면 됩니다.
...
-- FROM절 SUB QUERY를 활용한 범위 좁히기 개념에 대해서 보겠습니다.
-- FROM절 다음에는 테이블명만 올 수 있는게 아니고, 데이터 시트형식을 갖춘 자료라면 
-- 모든 형태의 자료를 적어줄 수 있습니다.

-- user_tbl 중 키가 170 미만인 요소만 1차적으로 조회한 결과시트를 토대로 전체 데이터 조회 구문.
-- 서브쿼리를 FROM절에서 사용할 때는 무조건 별칭을 붙여야 합니다.
...
-- 2. 위의 결과로 얻은 170을 토대로 쿼리문에 다시 WHERE절로 집어넣어 조회.
SELECT *  FROM user_tbl WHERE user_height > 170;

-- 개션 방안: WHERE절에 user_height > (서브쿼리로 최지선의 키를 구해주세요.)
...
-- 위 코드를 참조해서 평균보다 체중이 낮은 사람만 조회하는 쿼리문을 서브쿼리로 만들어주세요.
